...
# pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'  # 替换为实际的tesseract安装路径，有该语句时可以不用设置环境变量
#
#
# def pdf_to_images(pdf_path):
#     """将 PDF 转换为图像"""
#     return convert_from_path(pdf_path, poppler_path=r'C:\poppler-xx\Library\bin')  # 替换为实际poppler的安装路径,有该语句时也可以不用设置环境变量
#
#
...
#
#     output_text_file = os.path.splitext(pdf_path)[0] + '.txt'  # 输出文件名和输入文件名相同，增加.txt文件后缀
...
#
# # 设置 Tesseract 的路径（如果需要）
# pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'  # 替换为实际的tesseract安装路径，如果已经在环境变量中设置可省略
#
#
# # 将 PDF 转换为图像
# def pdf_to_images(pdf_path):
...
#
#     # 输入自定义文件名前缀
#     custom_filename = st.text_input("自定义输出 TXT 文件名称（不包含扩展名）", 'output')
#
#     if uploaded_file:
#         # 创建临时目录存放文件
